# Engagement Lifecycle — Developer Spec v2
...
**Changes from v1:** Deal ping timing corrected (supplier acceptance now precedes buyer results); Instant Book path added; Multi-property tour coordination documented; Q&A flow fully specced; WEx Guarantee defined; Supplier terms acceptance logic added.
...
**Buyers:** Sign during the commitment flow — Step 3 of the accept-to-tour sequence (see Section 3.3). Checkbox + timestamp + IP address. Must occur before address is revealed.
...
contact_captured      → Buyer provided email + phone
guarantee_signed      → Buyer signed WEx Occupancy Guarantee
...
| buyer_accepted → contact_captured | Buyer submits email + phone | 10 min (session) | Form stays open, no auto-cancel |
| contact_captured → guarantee_signed | Buyer checks guarantee agreement box | 10 min (session) | Form stays open, no auto-cancel |
...
Engagement:
  ...
  status: enum [
    ...
    matched, buyer_reviewing, buyer_accepted, contact_captured,
  ...
  # Contact (captured during buyer commitment flow)
  buyer_email: string (nullable)
  buyer_phone: string (nullable)
  contact_captured_at: timestamp (nullable)
...
EngagementEvent:
  ...
  event_type: enum [
    ...
    status_changed,
    contact_captured, guarantee_signed, address_revealed,
...
│  [________________________]                     │
│                                                 │
│  We'll use these to confirm your tour and       │
│  keep you updated on this space.                │
│                                                 │
│  [ Continue ]                                   │
│                                                 │
└─────────────────────────────────────────────────┘
...
**What happens:**
- Engagement status: buyer_accepted → contact_captured
- buyer_email, buyer_phone, contact_captured_at stored
...
**What happens:**
- Engagement status: contact_captured → guarantee_signed
...
This section is new in v2. Q&A is triggered when a buyer selects "I have questions" after a tour. Questions are routed by AI first, then to the supplier if the AI can't confidently answer. All answers are stored permanently in the property record.
...
│  YOUR ENGAGEMENTS                                   │
│  ┌───────────────────────────────────────────────┐  │
│  │ #1234 · North Charleston, SC                  │  │
│  │ 5,000 sqft · Storage · $4,250/mo              │  │
│  │ Status: Tour confirmed — Mar 5                │  │
│  │ [View Details]                                │  │
│  └───────────────────────────────────────────────┘  │
│                                                     │
│  ACTIVE SEARCHES                                    │
│  ┌───────────────────────────────────────────────┐  │
│  │ 10,000 sqft · Distribution · Houston, TX      │  │
│  │ 2 spaces being confirmed                      │  │
│  │ [View Matches]                                │  │
│  └───────────────────────────────────────────────┘  │
│                                                     │
└─────────────────────────────────────────────────────┘
...
### 11.3 Engagement Detail Page

```
/buyer/engagements/[id]

┌─────────────────────────────────────────────────────┐
│  ENGAGEMENT #1234                                   │
│  7704 Southrail Rd, North Charleston, SC            │
│  5,000 sqft · Storage · $4,250/mo                   │
│                                                     │
│  STATUS: Tour confirmed                             │
│  Tour: Tuesday, March 5, 10:00 AM                   │
│  [Get Directions]                                   │
│                                                     │
│  ────────────────────────────────────────────────── │
│                                                     │
│  TIMELINE                                           │
│  ✓ Mar 1 — Match found                              │
│  ✓ Mar 1 — Tour scheduled                           │
│  ● Mar 2 — Tour confirmed by property owner         │
│  ○ Mar 5 — Tour (upcoming)                          │
│                                                     │
│  ────────────────────────────────────────────────── │
│                                                     │
...
### 12.1 Deal Ping View (New in v2)

The supplier's first interaction with the platform for Tier 2 or cold-outreach suppliers is the deal ping. They may not have a full account yet. The SMS/email link should deep-link to a simple mobile-optimized confirmation page — no login required for the initial YES/NO response.

```
┌─────────────────────────────────────────────────┐
│                                                 │
│  WEx Deal — Warehouse Rental Opportunity        │
│                                                 │
│  A buyer needs:                                 │
│  5,000 sqft · Storage · 6 months               │
│  Your payout: $3,550/mo · $21,300 total         │
│                                                 │
│  [Review WEx Platform Terms]                    │
│                                                 │
│  By clicking Accept, you agree to WEx terms     │
│  for this and future engagements.               │
│                                                 │
│  [✓ Accept This Deal]   [✗ Pass]               │
│                                                 │
│  Responding via SMS also works:                 │
│  Reply YES to accept or NO to pass.             │
│                                                 │
│  Offer expires in: 11h 23m                      │
│                                                 │
└─────────────────────────────────────────────────┘
```

After accepting, supplier is prompted to create a full WEx account (or log in if they already have one) to track the engagement progress.

### 12.2 Supplier Engagement Views

(Existing spec preserved — supplier dashboard views of engagement at each stage remain as specced in v1. Add these new states to the supplier dashboard status display.)

| Stage | Supplier Sees |
|-------|--------------|
| **deal_ping_sent** | Deal ping notification (SMS/email only; not yet visible in dashboard) |
| **deal_ping_accepted** | Dashboard: "Deal confirmed — waiting for buyer to schedule tour" |
| **instant_book_requested** | Dashboard: "Buyer has instant-booked your space — agreement coming" |
| All other states | Same as v1 |
...
|--------|------|
| ID | Engagement ID |
| Parties | Buyer name, Supplier name, Property |
| Status | Current state |
| Tier | Tier 1 / Tier 2 |
| Path | Tour / Instant Book |
| Terms Status | Supplier terms accepted? |
| Monthly | Buyer / Supplier / WEx amounts |
| Stage Duration | Time in current state |
| Created | Date created |
| Next Deadline | When current stage expires |

**Filters:** Status (multi-select), tier, path, date range, property, buyer, supplier, stale (same state > X days), supplier_terms_pending
...
### Accept-to-Tour / Instant Book
| Method | Path | Description |
|--------|------|-------------|
| POST | /api/engagements/{id}/accept | Buyer accepts match → buyer_accepted. Body: {path: "tour" \| "instant_book"} |
| POST | /api/engagements/{id}/contact | Buyer submits email + phone → contact_captured |
...
| **buyer_accepted** | Contact capture form | Dashboard: "Buyer is committing to this space" |
| **contact_captured** | Guarantee agreement | Nothing changed |
