...
export default function Phase6AddAsset(props: Phase6AddAssetProps) {
  const { truthCore, buildingData, setMemories } = props;
  const router = useRouter();
  const [isActive, setIsActive] = useState(false);
  const [activating, setActivating] = useState(false);

  // Agreement only — no credentials needed
  ...
  // Read existing supplier profile
  const supplier = (() => {
    try {
      const raw = localStorage.getItem('wex_supplier');
  ...
  async function handleActivate() {
    ...
    addMemory(
      'Activation',
      `New asset added: ${truthCore.sqft.toLocaleString()} sqft @ $${truthCore.rateAsk.toFixed(2)}/sqft/mo → $${revenue.toLocaleString()}/yr`,
      '🏭'
    );
          ...
          <div className="relative" style={{ height: '45%', minHeight: 200 }}>
            ...
            <div className="absolute top-4 right-4 z-20">
              ...
              >
                ...
                {isActive ? 'Asset Active' : 'Adding to Portfolio'}
              </motion.div>
            </div>
          ...
          <div className="px-8 pt-6 pb-8 bg-white">
            ...
            {supplier?.name && (
              <p className="text-slate-500 text-sm text-center mb-4">
                Adding to <span className="font-semibold text-slate-700">{supplier.name}</span>&apos;s portfolio
              </p>
            )}
              ...
              <div>
                ...
                <label className="flex items-start gap-3 cursor-pointer mb-5 group">
                  ...
                  <span className="text-slate-600 text-xs leading-relaxed group-hover:text-slate-800 transition-colors">
                    ...
                    to list this property and facilitate tenant placements on my behalf.
                  </span>
                </label>
                ...
                <motion.button
                  ...
                  animate={{
                    backgroundColor: agreementAccepted ? '#059669' : '#94a3b8',
                    boxShadow: agreementAccepted
                ...
                >
                  <ShieldCheck size={20} />
                  {activating ? 'Activating...' : 'Add Asset to Portfolio'}
                </motion.button>

                <p className="text-xs text-slate-400 mt-4 max-w-xs mx-auto text-center">
                  By clicking, you accept the WEx Capacity Agreement for this property.
                </p>
              </div>
            ) : (
              <motion.div
                ...
                <div className="space-y-4">
                  <div className="bg-emerald-50 border border-emerald-100 rounded-xl p-4 text-center">
                    ...
                    <p className="text-emerald-600 text-sm">
                      Matching tenants to your new property...
                    </p>
                  </div>
