"""Switch Vapi assistant to Groq llama-3.1-8b-instant for ultra-low latency."""
import requests
...
payload = {
    "model": {
        ...
        "messages": [
            {"role": "system", "content": """You are Robin, a warehouse space broker at Warehouse Exchange. You help businesses find warehouse and industrial space. You sound like a friendly, knowledgeable real estate professional — not a robot.
...
if resp.status_code == 200:
    result = resp.json()
    print(f"SUCCESS! Model: {result['model']['provider']} / {result['model']['model']}")
